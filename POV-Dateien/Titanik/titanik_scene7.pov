...
object {Stockwerk translate <0,1,-1> scale <0,0,0.9>}
    ...
    #if (clock > 0.5 & clock < 1)
    rotate <2.5+85*(clock-0.5)*2,0,0>
    #else   
    rotate <5*clock,0,0>
    #end
    
    #if (clock < 0.9)
    translate <0,-0.5*clock,2>
    #else
    translate <0,-0.5-20*(clock-0.9)*5,0>
    #end 
...
#if (clock > 0.35 & clock < 0.4)
#declare Man_Scale = <3,3,3>;
#else 
#if (clock > 0.4)
#declare Man_Scale = <0.01,0.01,0.01>; 
#end
#end
